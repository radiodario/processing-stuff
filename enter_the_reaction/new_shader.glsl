...
void main()
{
    ...
    float oldU = texture2D(tex0U, vertTexCoord.st).r;
    float oldV = texture2D(tex0U, vertTexCoord.st).g;

    // [rad] Compute approximation of Laplacian for both V and U.
    float otherU = -4.0 * oldU;
    float otherV = -4.0 * oldV;

    otherU += texture2D(tex0U, gl_TexCoord[0].xy + vec2(-texOffset.s  , 0)).r;
    otherU += texture2D(tex0U, gl_TexCoord[0].xy + vec2(texOffset.s  , 0)).r;
    otherU += texture2D(tex0U, gl_TexCoord[0].xy + vec2(0, -step_height)).r;
    otherU += texture2D(tex0U, gl_TexCoord[0].xy + vec2(0, step_height)).r;

    otherV += texture2D(tex0U, gl_TexCoord[0].xy + vec2(-texOffset.s  , 0)).g;
    otherV += texture2D(tex0U, gl_TexCoord[0].xy + vec2(texOffset.s  , 0)).g;
